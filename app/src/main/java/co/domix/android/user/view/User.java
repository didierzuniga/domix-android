...
public class User extends AppCompatActivity implements UserView, GoogleApiClient.OnConnectionFailedListener,
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (getIntent().getIntExtra("earnedCredit", 0) != 0){
            View parentLayout = findViewById(android.R.id.content);
            Snackbar snk = Snackbar.make(parentLayout, getString(R.string.text_accumulated_beam) +
                    " " +
                    getIntent().getIntExtra("earnedCredit", 0) +
                    " " +
                    getIntent().getStringExtra("currencyCode"),
                    Snackbar.LENGTH_LONG);
            View snackBarView = snk.getView();
            snackBarView.setBackgroundColor(getResources().getColor(R.color.button_green_light));
            TextView textView = snackBarView.findViewById(android.support.design.R.id.snackbar_text);
            textView.setTextSize(TypedValue.COMPLEX_UNIT_PX, getResources().getDimension(R.dimen.textSnackbar));
            textView.setTextColor(Color.WHITE);
            snk.setDuration(4000);
            snk.show();
        }
